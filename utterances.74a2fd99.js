...
var UTTERANCES_API = 'https://guides-comments2.azurewebsites.net';
...
function handleTextAreaResize(textarea) {
  ...
  var track = function track() {
    _this.addEventListener('mousemove', _measure.scheduleMeasure);

    _this.addEventListener('mouseup', stopTracking);
  };
...
function loadTheme(theme, origin) {
  ...
  return new Promise(function (resolve) {
    ...
    _this.addEventListener('message', function (event) {
...
bootstrap();
(void 0).addEventListener('not-installed', function handleNotInstalled() {
